...
系统不在此页给出买卖排名。16:50 `factcard_refresh` 只在相对上一时点有重要变化时推送摘要 + 证据编号。
...
| 交易日 17:00 | 晚间盘后报告 | 一天只这一份（2026-09-07 由 22:00 提前，配合机器 17:30 后休眠） |
| 周日 20:00 | 周报 | 消息面 + 周度复盘 |
| 交易日 16:50 | 事实卡变化 | 仅重要变化 + `EVID-` 编号 |
...
| 交易日 16:20 | pool_trap_scan | 候选池杀猪盘 |
| 交易日 16:30 | industry_refresh | 行业数据 + 定量 |
| 交易日 16:40 | daily_refresh | 日线/指数补采（历史权威；当日以 15:01 快照为准） |
| 交易日 16:50 | factcard_refresh | 事实卡；仅推重要变化 |
| 交易日 17:00 | evening_report | 晚间盘后报告；**补偿按交易日**，周末不补发 |
| 周日 20:00 | weekend | 周报 + 周度复盘 |
| 每月 1 日 09:30 | monthly | 月度复盘 |
| 每年 1 月 1 日 09:30 | yearly | 年度复盘 |

电源/唤醒（2026-09-07 起，机器仅交易日 08:30-17:30 保持唤醒，其余时间允许休眠省电）：
- `TraderSystem_power_on`：交易日 08:25（WakeToRun 唤醒 + 启动 `scripts/keep_awake.py` 持有 ES_SYSTEM_REQUIRED 保活至 17:30，阻止"唤醒后无人值守自动复睡"——2026-09-08 曾因 08:26:56 复睡漏跑 premarket）
- `TraderSystem_power_off`：交易日 17:35（恢复插电 25 分钟休眠）
- `InvestSystemWake`：08:25/15:55/21:25/21:58 周期性唤醒并守护调度服务（wake_guard.ps1：服务未跑则拉起、job_runs 冻结 >8h 则重启服务）
...
| P1 | 收盘扫描变化 | 600s |
| P2 | 17:00 晚报 | 600s；日线未到最近交易日则只推滞后原因 |
